...
-- ============================================================
-- Helper function for RLS: check project membership
-- ============================================================
...
-- Auto-insert owner as member when project is created
...
create policy "Members can manage characters"
...
create policy "Members can manage relationships"
...
create policy "Members can manage episodes"
...
-- Acts inherit access through episodes → projects
create policy "Members can view acts"
  on public.acts for select
  using (
    exists (
      select 1 from public.episodes e
      where e.id = episode_id
        and public.is_project_member(e.project_id)
    )
  );

create policy "Members can manage acts"
...
create policy "Members can view beats"
...
create policy "Members can manage beats"
...
create policy "Members can manage discussions"
...
create policy "Members can view memory"
  on public.memory_index for select
  using (public.is_project_member(project_id));

create policy "Members can manage memory"
  on public.memory_index for all
  using (public.is_project_member(project_id));
...
create policy "Members can view bible sections"
  on public.show_bible_sections for select
  using (public.is_project_member(project_id));

create policy "Members can manage bible sections"
  on public.show_bible_sections for all
  using (public.is_project_member(project_id));
...
create policy "Members can view bible versions"
  ...
  using (
    exists (
      select 1 from public.show_bible_sections s
      where s.id = bible_id
        and public.is_project_member(s.project_id)
    )
  );

create policy "Members can insert bible versions"
  on public.bible_versions for insert
  with check (
    exists (
      select 1 from public.show_bible_sections s
      where s.id = bible_id
